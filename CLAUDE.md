...
Wellbin Code is a terminal AI coding assistant CLI. Large parts of the tree use feature flags and optional modules; some areas may still carry loose typings from historical tooling. Typecheck noise does **not** necessarily block Bun runtime — prioritize runtime behavior and targeted tests when changing behavior.
...
- **`src/query.ts`** — The main API query function. Sends messages to the configured model API, handles streaming responses, processes tool calls, and manages the conversation turn loop.
- **`src/QueryEngine.ts`** — Higher-level orchestrator wrapping `query()`. Manages conversation state, compaction, file history snapshots, attribution, and turn-level bookkeeping. Used by the REPL screen.
- **`src/screens/REPL.tsx`** — The interactive REPL screen (React/Ink component). Handles user input, message display, tool permission prompts, and keyboard shortcuts.

### API Layer

- **`src/services/api/claude.ts`** — Core API client. Builds request params (system prompt, messages, tools, betas), calls the provider streaming endpoint, and processes `BetaRawMessageStreamEvent` events.
- Supports multiple backends: direct API, AWS Bedrock, Google Vertex, Azure, and compatible proxies.
...
- Components use React Compiler runtime (`react/compiler-runtime`) — build output may include `_c()` memoization calls throughout.
...
- CLI 快速路径: `wellbin remote-control` / `wellbin rc` / `wellbin bridge`（以实际注册的子命令为准）。
...
- **`src/types/global.d.ts`** — Declares `MACRO`, `BUILD_TARGET`, `BUILD_ENV` and other build-time identifiers.
...
- **Don't try to fix all tsc errors** — many stem from legacy typing and don't affect runtime.
- **Feature flags** — 默认全部关闭（`feature()` 返回 `false`）。Dev/build 各有自己的默认启用列表。不要在 `cli.tsx` 中重定义 `feature` 函数。
- **React Compiler output** — Components may include memoization boilerplate (`const $ = _c(N)`). This is normal.
...
- **Biome 配置** — 大量 lint 规则被关闭（历史代码体量下不宜一刀切严格 lint）。`.tsx` 文件用 120 行宽 + 强制分号；其他文件 80 行宽 + 按需分号。
