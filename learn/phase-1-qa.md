...
#### 场景 1：`wellbin --version`（命中快速路径）

```
cli.tsx main() 开始执行
  ├── args = ["--version"]
  ├── 命中第 64 行: args[0] === "--version" ✅
  ├── console.log("2.1.888 (Wellbin Code)")
  └── return  ← 立即退出，零 import，~10ms
```

#### 场景 2：`wellbin --claude-in-chrome-mcp`（命中中间路径）
...
| | `claude` | `claude -p` | `claude -c` | `wellbin mcp list` |
|---|---------|------------|------------|-------------------|
