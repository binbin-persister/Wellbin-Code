...
原版 Wellbin Code（上游）构建时，Bun bundler 通过 `bun:bundle` 提供 `feature()` 函数，用于**编译时 feature flag**（类似 C 的 `#ifdef`）。反编译版没有构建流程，所以直接定义为永远返回 `false`。
...
| 子命令 | 行号 | 作用 |
|--------|------|------|
| `wellbin mcp` | 3892 | MCP 服务器管理（serve/add/remove/list/get） |
| `wellbin server` | 3960 | Session 服务器（❌ flag 关闭） |
| `wellbin auth` | 4098 | 认证管理（login/logout/status/token） |
| `wellbin plugin` | 4148 | 插件管理（install/uninstall/list/update） |
| `wellbin setup-token` | 4267 | 设置长期认证 token |
| `wellbin agents` | 4278 | 列出已配置的 agents |
| `wellbin doctor` | 4346 | 健康检查 |
| `wellbin update` | 4362 | 检查更新 |
| `wellbin install` | 4394 | 安装原生构建 |
| `claude log` | 4411 | 查看对话日志（内部） |
| `wellbin completion` | 4491 | Shell 自动补全 |
