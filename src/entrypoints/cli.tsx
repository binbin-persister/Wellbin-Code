...
async function main(): Promise<void> {
  ...
  if (args.length === 1 && (args[0] === '--version' || args[0] === '-v' || args[0] === '-V')) {
    ...
    console.log(`${MACRO.VERSION} (Wellbin Code)`);
    return;
  }
  ...
  // Fast-path for `wellbin remote-control` (also accepts legacy `wellbin remote` / `wellbin sync` / `wellbin bridge`):
  ...
  // Fast-path for `wellbin daemon [subcommand]`: long-running supervisor.
  ...
  // Fast-path for `wellbin ps|logs|attach|kill` and `--bg`/`--background`.
  ...
  // Fast-path for `wellbin environment-runner`: headless BYOC runner.
  // feature() must stay inline for build-time dead code elimination.
  ...
  // Fast-path for `wellbin self-hosted-runner`: headless self-hosted-runner
