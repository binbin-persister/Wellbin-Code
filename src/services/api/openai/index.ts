...
> {
  ...
  try {
    ...
    // Hold the slot for the full lifetime of the streaming request so
    // low-concurrency OpenAI-compatible gateways do not see overlapping
    // in-flight streams from the same process.
    releaseRequestSlot = await acquireOpenAIRequestSlot(signal)
    const stream = await retryOpenAIRequest(
      () =>
