 ...
 * who runs `wellbin` in their terminal with an API key sees every CCD session
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // `wellbin ssh` remote: ANTHROPIC_UNIX_SOCKET tunnels API calls through a
...
export function getAnthropicApiKeyWithSource(
  ...
  // Always check for direct environment variable when the user ran wellbin --print.
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  // `wellbin ssh` remote: real auth lives on the local machine and is injected
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from 'wellbin setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via 'wellbin auth login'.`,
    }
  }
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: wellbin auth login`,
  }
}
