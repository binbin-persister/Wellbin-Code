...
/**
 * Kind override from env. Set by the spawner (`wellbin --bg`, daemon
...
/**
 * True when this REPL is running inside a `wellbin --bg` tmux session.
 ...
 * typescript, python, -p), bg/daemon spawns — so `wellbin ps` sees everything
...
export async function registerSession(): Promise<boolean> {
  ...
  try {
    ...
    // this, the PID file's sessionId goes stale and `wellbin ps` sparkline
...
/**
 * Push live activity state for `wellbin ps`. Fire-and-forget from REPL's
