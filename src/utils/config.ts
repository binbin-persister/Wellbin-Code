...
export type ProjectConfig = {
  ...
  /** Spawn mode for `wellbin remote-control` multi-session. Set by first-run dialog or `w` toggle. */
...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when Wellbin Code was first started on this machine
  ...
  // Wellbin Code usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first Wellbin Code OAuth token
  ...
  // Wellbin Code hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
  ...
  // cold API call on every `wellbin -p` invocation.
  metricsStatusCache?: {
    enabled: boolean
    timestamp: number
  }
