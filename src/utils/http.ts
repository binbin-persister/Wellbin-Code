...
import { getWellbinCodeUserAgent } from './userAgent.js'
import { getWorkload } from './workloadContext.js'

// WARNING: We rely on `wellbin-cli` in the user agent for log filtering.
...
export function getUserAgent(): string {
  ...
  return `wellbin-cli/${MACRO.VERSION} (${process.env.USER_TYPE}, ${process.env.CLAUDE_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}
...
export function getMCPUserAgent(): string {
  ...
  return `wellbin-code/${MACRO.VERSION}${suffix}`
}
...
// operators match in robots.txt); the wellbin-code suffix distinguishes
// local CLI traffic from claude.ai server-side fetches.
export function getWebFetchUserAgent(): string {
  return `Claude-User (${getWellbinCodeUserAgent()}; +https://support.anthropic.com/)`
}
